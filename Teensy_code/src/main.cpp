...
LedStrip ledLEFT(LED_LEFT, 5);
LedStrip ledMIDDLE(LED_MIDDLE, 5);
LedStrip ledRIGHT(LED_RIGHT, 5);
ServoKit servoKit;
...
static void handleSwitchObstacle(SwitchEvent ev) {
  ...
  robotBase.stop(true);
  ledMIDDLE.setColor(LedSide::ALL, LedColor::RED);
...
void setup() {
  ...
  if (digitalRead(BUTTON_PIN) == LOW) {
    ...
    if (mode == 't' || mode == 'a') {
      ...
      ToFCalibration::run(tof, BUTTON_PIN, ledMIDDLE);
    }
    if (mode == 'c' || mode == 'a') {
  ...
  ledLEFT.setColor(LedSide::ALL, LedColor::WHITE);
  ledRIGHT.setColor(LedSide::ALL, LedColor::WHITE);
...
void loop() {
  ...
  if (pauseByButton) {
    ...
    if (millis() - pauseLedToggleMs >= PAUSE_LED_INTERVAL_MS) {
      ...
      if (pauseLedOn) ledMIDDLE.setColor(LedSide::ALL, LedColor::RED);
      else ledMIDDLE.clear();
    }
  ...
  if (canActVictim && (victimLeft || victimRight)) {
    ...
    robotBase.stop(true);
    ledMIDDLE.setColor(side, LedColor::YELLOW);
